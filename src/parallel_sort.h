...
const size_t SEQUENTIAL_SORT_SIZE_THRESHOLD = 100; // Длина подмассива, для которого необходимо вызвать последовательный алгоритм.

template <typename T>
class SortTask {
private:
    std::vector<T>* data;
    TasksQueue<std::vector<T>>* tasks;
    ...
    size_t partition(size_t left_index, size_t right_index) {
        ...
        return i;    const size_t SEQUENTIAL_SORT_SIZE_THRESHOLD = 100000;
    }
       
public:
    SortTask(std::vector<T>& data_set, TasksQueue<std::vector<T>>& task_set) : decomposition_level(SEQUENTIAL_SORT_SIZE_THRESHOLD) {
        data = &data_set;
        tasks = &task_set;
    }
